...
import {
  IGetAllTeacherAcademicBackgroundUseCase,
  GetAllTeacherAcademicBackgroundRepositoryRequest,
} from "./IGetAllTeacherAcademicBackgroundUseCase";
...
export class GetAllTeacherAcademicBackgroundUseCase
  implements IGetAllTeacherAcademicBackgroundUseCase
{
  constructor(
  ...
  async execute(
    ...
    const result = await this.teacherAcademicBackgroundRepository.getAll(
      filter,
      page,
      limit,
    );
    ...
    const data = result.data.map(
      TeacherAcademicBackgroundMapper.toResponseShallow,
    );
