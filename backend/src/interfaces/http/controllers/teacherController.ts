...
import {GetAllSectionUseCase} from "../../../application/use-cases/section";
import {FilterSection} from "../../../application/use-cases/section/GetAllSectionUseCase";
...
export const getMySections = async (req: AuthenticatedRequest, res: Response) => {
    const teacherId = req.user!.employeeId!;
    const {schoolYear} = req.query;


    const filter: FilterQuery<FilterSection> = {adviserId: teacherId};

    if (schoolYear && typeof schoolYear === "string") {
        filter.schoolYear = schoolYear;
    }

    const sections = await getAllSectionUseCase.execute(filter, 0, 10);
